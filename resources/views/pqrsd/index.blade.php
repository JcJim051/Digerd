        ...
        <div class="panel panel-bordered">
            <div class="panel-body">
                <div class="table-responsive">
                    <table id="dataTableRespuestaPQRSD" name="dataTableRespuestaPQRSD"
                            ...
                            @foreach ($pqrsds as $pqrsd)
                                <tr role="row">
                                    ...
                                    <td class="text-center">
                                        <a href="{{ Storage::disk(config('voyager.storage.disk'))->url(json_decode($pqrsd->soporte_solicitud)['0']->download_link) ?:'' }}"
                                            title="Soporte" target="_blank" class="btn btn-sm btn-warning pull-right view">
                                    ...
                                    <td class="text-center">
                                        @if ($pqrsd->estado != 2)
                                            @if ($pqrsd->plazo < 0)
                                                <p class="text-danger font-weight-bold h4">Vencido hace <?php echo abs($pqrsd->plazo); ?>
                                                    días</p>
                                            @elseif ($pqrsd->plazo == 0)
                                                <p class="text-warning font-weight-bold h4">Vence hoy</p>
                                            @elseif ($pqrsd->plazo > 0 && $pqrsd->plazo < 6)
                                                <p class="text-info font-weight-bold h4">{{ $pqrsd->plazo }} días</p>
                                            @elseif ($pqrsd->plazo > 5 && $pqrsd->plazo <= 10)
                                                <p class="text-success font-weight-bold h4">{{ $pqrsd->plazo }} días</p>
                                            @endif
                                        @else
                                            <p class="text-success font-weight-bold h4">----</p>
                                        @endif
                                        {{-- <td class="text-center">
                                        {{ $pqrsd->plazo < 0? 'Vencido': ($pqrsd->soporte_respuesta == '[]'? ($d[0] == '1'? $d[0] . ' día': $d[0] . ' días'): '-') }}
                                    </td> --}}
                                    </td>
                                    <td class="text-center">
                                        @if ($pqrsd->estado == 2)
                                        ...
                                        @else
                                            <a href="{{ url('respuestaPQRSD/' . $pqrsd->id_pqrsd) }}" title="Editar"
                                                class="btn btn-sm btn-primary pull-right edit">
                                                <i class="voyager-edit"></i> <span class="hidden-xs hidden-sm">Dar
                                                    Respuesta</span>
                                            </a>
                                        @endif
